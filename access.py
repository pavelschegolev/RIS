...
def group_validation(config: dict) -> bool:
    endpoint_app = request.endpoint.split('.')[0] #request.endpoint - это атрибут объекта запроса (request), который в фреймворке Flask представляет текущий конечный путь запроса.
                                                # Возвращает   "bp_query.query_index1" после чего делает список и берет первый элемент
